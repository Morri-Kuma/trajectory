#!/usr/bin/env python3
"""
Build the GSE230659 post-QC raw/full-gene h5ad for milestone annotation.

This mirrors the GSE178325 raw/full-gene builder: post-QC cells, merged
gene-symbol sparse log-normalized full-gene X, and stable obs names matching
the existing HVG2000 benchmark input. The output is intended for marker-based
milestone annotation only; model training should continue to use HVG2000.
"""
...
PROCESSED_DIR = PROJECT_ROOT / "data" / "processed" / "gse230659_human"
OUTPUT_H5AD = PROCESSED_DIR / "GSE230659_0618_raw_full_gene_benchmark_input.h5ad"
...
def sample_path(sample_name: str) -> Path:
...
print(f"[{TIMESTAMP}] GSE230659 raw/full-gene builder")
...
for sample_name, stage, day in SAMPLES:
    t_start = _time.time()
    row, col, data, n_genes, n_cells, barcodes, features = read_10x_mtx(sample_path(sample_name))
    if n_genes != N_GENES_REF:
        raise ValueError(f"Gene count mismatch in {sample_name}: {n_genes} vs {N_GENES_REF}")
    ...
    print(
        f"  {sample_name}: {n_cells:>6,} -> {len(kept):>6,} cells "
        f"abs_day={abs_day:<5.2f} [{_time.time() - t_start:.1f}s, {rss_gb():.1f}GB]"
    )
...
var = pd.DataFrame(
    {
        ...
        "gene_symbol": np.asarray(UNIQUE_GENE_SYMBOLS, dtype=object)[symbol_indices],
        "gene_name": np.asarray(UNIQUE_GENE_SYMBOLS, dtype=object)[symbol_indices],
...
for sample_name, stage, day in SAMPLES:
    ...
    print(f"  {sample_name}: nnz={len(values):,} [{_time.time() - t_start:.1f}s, {rss_gb():.1f}GB]")
...
adata.uns["preprocessing_summary"] = {
    "script": "scripts/build_gse230659_raw_full_gene_input.py",
    "dataset": DATASET_ID,
    "batch_id": BATCH_ID,
    "timestamp": TIMESTAMP,
    "qc_params": QC_PARAMS,
    ...
    "intended_use": "full-gene marker-based milestone annotation; not model training",
}
