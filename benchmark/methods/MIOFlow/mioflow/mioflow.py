...
class MIOFlow:
    ...
    sample_size : int, optional
        Batch size (points sampled per time step). ``None`` 鈫?use all.
    n_trajectories : int
    ...
    def _encode(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Priority order:
        1. ``gaga_model`` 鈥?encodes ``adata.obsm[gaga_input_key]`` to latent space.
        2. Fallback 鈥?reads the raw array from ``adata.obsm[gaga_input_key]``.
    ...
    def decode_to_gene_space(self) -> np.ndarray:
        ...
        Requires:
        - ``gaga_model`` passed at construction (for latent 鈫?PCA decoding)
        - ``adata.varm['PCs']`` (for PCA 鈫?gene space)
        ...
        # Flatten (n_bins, n_traj, latent_dim) 鈫?(n_bins * n_traj, latent_dim) for batch decoding
        traj_shape = self.trajectories.shape
        traj_flat = self.trajectories.reshape(-1, traj_shape[-1])

        # GAGA latent 鈫?scaled PCA space
        self.gaga_autoencoder.eval()
        with torch.no_grad():
