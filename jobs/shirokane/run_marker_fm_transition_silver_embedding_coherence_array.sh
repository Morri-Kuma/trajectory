...
print(json.dumps({
    ...
    "entropy_basis": m.get("entropy_basis"),
}, indent=2))
...
if "n_neighbors=15:resolution=0.5" not in str(m.get("cluster_source", "")):
    raise SystemExit(f"unexpected cluster_source: {m.get('cluster_source')}")
if m.get("pred_tp_avg_normalized_entropy") is None:
    raise SystemExit("missing pred_tp_avg_normalized_entropy")
