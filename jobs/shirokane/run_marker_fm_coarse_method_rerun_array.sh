...
#$ -cwd
#$ -j y
#$ -o logs/run_marker_fm_coarse_method_rerun.$JOB_ID.$TASK_ID.log
#$ -l s_vmem=128G
#$ -t 1-20
...
RUN_TAG="${RUN_TAG:-marker_fm_coarse_rerun_$(date +%Y%m%d_%H%M%S)}"
...
echo "============================================================"
echo "Marker FM coarse method rerun"
...
echo "Purge model cache : ${PURGE_METHOD_CACHE}"
echo "============================================================"
...
read -r METHOD SCENARIO H5AD_PATH OUTPUT_DIR CELL_STATE_KEY REF_GRAPH < <(
  python - "${CONFIG}" <<'PY'
...
ref_graph = (
    ...
    or ground_truth.get("reference_graph_path")
    or ""
)
print(method, scenario, h5ad, output, cell_state_key, ref_graph)
PY
)
...
try:
    ...
    if stage:
        raise SystemExit(f"coarse h5ad labels contain stage labels: {stage[:20]}")
finally:
    adata.file.close()

graph = json.loads(ref_graph.read_text(encoding="utf-8"))
nodes = {str(n["id"]) for n in graph.get("nodes", [])}
stage_nodes = sorted(x for x in nodes if x.startswith("stage_"))
if stage_nodes:
    raise SystemExit(f"reference graph contains stage labels: {stage_nodes[:20]}")
...
print(f"[OK] preflight labels: {config.name}, h5ad labels={sorted(labels)}, nodes={sorted(nodes)}")
PY
...
for path in (stm_path, edges_path, metrics_path):
    if not path.exists():
...
stage = sorted({x for x in rows + cols if x.startswith("stage_")})
if stage:
    raise SystemExit(f"state_transition_matrix.csv still uses stage labels: {stage[:20]}")
missing_rows = [x for x in ref_nodes if x not in set(rows)]
missing_cols = [x for x in ref_nodes if x not in set(cols)]

edge_text = edges_path.read_text(encoding="utf-8", errors="replace")
if "stage_" in edge_text:
    raise SystemExit("lineage_graph_edges.csv still contains stage_ labels")
...
print(
    "[OK] scTimeBench zero-fill coarse lineage outputs verified: "
    f"{out_dir}; missing_reference_nodes={sorted(reported_missing)}"
)
PY

echo "============================================================"
echo "Marker FM coarse method rerun finished at: $(date)"
echo "Config: ${CONFIG}"
echo "============================================================"
