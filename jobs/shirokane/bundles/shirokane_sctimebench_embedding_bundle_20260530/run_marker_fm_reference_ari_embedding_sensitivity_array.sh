...
#$ -j y
#$ -t 1-54
#$ -o logs/run_marker_fm_reference_ari_embedding_sensitivity.$JOB_ID.$TASK_ID.log
#$ -l s_vmem=64G
...
echo "============================================================"
echo "Marker-FM reference-ARI embedding sensitivity evaluation"
...
MAX_TASKS=$((N_CONFIGS * N_RESOLUTIONS))
...
if [ "${TASK_ID}" -lt 1 ] || [ "${TASK_ID}" -gt "${MAX_TASKS}" ]; then
  echo "Task ${TASK_ID} is outside sensitivity range 1-${MAX_TASKS}; nothing to do."
  exit 0
fi

ZERO_BASED=$((TASK_ID - 1))
RES_IDX=$((ZERO_BASED / N_CONFIGS))
CONFIG_IDX=$((ZERO_BASED % N_CONFIGS))
LEIDEN_RESOLUTION="${RESOLUTIONS[${RES_IDX}]}"
RES_TAG="${LEIDEN_RESOLUTION/./_}"
...
EMBEDDING_OUT="${OUTPUT_DIR}/embedding_milestone_eval_resolution_sensitivity/resolution_${RES_TAG}"
...
echo "------------------------------------------------------------"
echo "Run expanded_sensitivity Embedding Coherence reference ARI"
echo "------------------------------------------------------------"
python -m benchmark.evaluation.eval_embedding_milestone \
  ...
  --dataset-id "${DATASET}" \
  --mode run-output \
  --label-mode expanded_sensitivity \
...
echo "------------------------------------------------------------"
echo "Validate sensitivity reference-ARI fields"
echo "------------------------------------------------------------"
...
metrics_path = out / "embedding_metrics_expanded_sensitivity.json"
...
echo "============================================================"
echo "Reference-ARI embedding sensitivity finished at: $(date)"
echo "Output: ${EMBEDDING_OUT}/embedding_metrics_expanded_sensitivity.json"
echo "============================================================"
