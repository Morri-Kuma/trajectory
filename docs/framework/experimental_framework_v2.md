...
## 1.1 Scope update v2.1 (2026-06-10)

This revision refines the benchmark and **removes three axes** from the active
design (deleted here and in the manuscript/design deliverables):

1. **Input representation (HVG2000 vs scGPT-CLS vs Geneformer-CLS).** The
   representation-dynamics study is retired from the active benchmark; the only
   model input is 2,000-HVG expression. The two representation experimental-record
   entries formerly at the end of this file are removed.
2. **Annotation-provider-as-condition.** The benchmark uses a single frozen
   `official_silver` provider per dataset as the fixed reference; it is no longer
   treated as a replaceable/sensitivity condition, and the
   consensus/embedding/classifier provider comparison is not part of the design.
3. **Time-library confounding.** Removed as a framing/limitation axis (former
   Section 4.4).

**Two innovations now distinguish this benchmark from scTimeBench
(Osakwe et al. 2026):**

- **(I) A pluripotency-domain cross-modality generalization benchmark.** At least
  six time-labeled iPSC scRNA-seq datasets spanning chemical reprogramming, OSKM
  (transcription-factor) reprogramming, iPSC-directed differentiation, mouse
  MEF$\to$iPSC reprogramming, and (as the stated extension) a human cerebral-organoid
  time course. The headline question is whether a method's ranking *transfers*
  across reprogramming routes that converge on the same pluripotent endpoint, not
  a single averaged leaderboard.
- **(II) A reference-construction-and-validation pipeline.** Because most iPSC
  datasets ship without per-cell author labels, silver references are built by
  scANVI/scArches label transfer from an annotated dataset and **gated by a
  geometric out-of-distribution (OOD) score**; cross-system transfers that the OOD
  score flags are rejected, and only within-modality transfers anchor
  lineage/embedding references. This integrates the project's scANVI work as the
  benchmark's annotation engine and trust gate (see the design doc).

The three core scTimeBench tasks, the method roster and capability gating, the
A/B/C scenarios, and the frozen `official_silver` provider mechanism are otherwise
unchanged.
...
*(v2.1, 2026-06-10) The former time-library-confounding limitation discussion has
been removed from the active design (see the v2.1 scope note in Section 1.1).*
General per-dataset QC is retained: report leave-one-timepoint-out metric stability
for Scenarios A–C where feasible, and inspect the smallest-post-QC-count and
highest-low-quality-rate samples per dataset before formal runs.
...
`gse242424_oskm_reprogramming_ground_truth_v1`, based on author-cluster matched OSKM
...
Full annotation jobs completed on Shirokane for both datasets and were synchronized locally. GSE178325 produced a trajectory-dominant official ground truth label distribution of approximately 67,817 `epithelial_like`, 5,706 `hADSCs`, 4,654 `intermediate_plastic`, 2,144 `hCiPS`, and 154 `xen_like` cells. GSE230659 produced approximately 67,112 `epithelial_like`, 5,370 `hCiPS`, and 2,712 `intermediate_plastic` cells; no `hADSCs` were assigned there because the available earliest time point was 0.5 rather than 0. All synchronized annotation outputs contained no `failure_or_offtrajectory`, `out_of_trajectory`, or `marker_diagnostic` labels.
...
**2026.05.22 - Expanded official-silver marker-FM rerun**
GSE230659/GSE178325 annotation policy was revised so `hADSCs` means marker-defined hADSC-like rather than sample/time-forced, and low-margin adjacent milestone mixtures are deferred from Stage 1 to Stage 2.
Official marker-FM metrics were promoted from `final_milestone_label_coarse` to `final_milestone_label_expanded`; rebuilt providers now use GSE178325 9 states/8 edges and GSE230659 7 states/6 edges.
Runtime configs, evaluators, validation scripts, and Shirokane launchers were updated so Lineage Fidelity and Embedding Coherence consume expanded official-silver labels.
The expanded-official Shirokane rerun completed for MIOFlow, PRESCIENT, WOT, CellRank2, and all 18 projection post-embedding tasks.
scNODE lineage metrics were repaired posthoc from existing STM/edge outputs after an import-path failure; final audit: 20/20 lineage metrics and 18/18 projection post-metrics complete, all using expanded labels.

**2026.05.28 - Official-silver contract reconciliation**
The expanded-label/reference-ARI embedding rerun is retained as an `expanded_sensitivity` analysis rather than formal official output because the synchronized frozen registry, runtime configs, and lineage metrics still declare `final_milestone_label_coarse`. Formal Embedding Coherence was rebuilt under `final_milestone_label_coarse` with Leiden `n_neighbors=15` and `resolution=1.0`, matching the existing lineage state system. `benchmark/evaluation/summarize_official_silver.py` now validates registry, embedding, lineage, and Leiden parameters before writing any combined report, so expanded or alternative-resolution sensitivity outputs cannot be silently mixed into official rankings.

---

**2026.5.23-5.31 - Weekly experimental record**
Date note: 5.23-5.31.

The week centered on resolving the metric-contract question for official silver output. The expanded-label/reference-ARI embedding rerun was kept as `expanded_sensitivity`, while formal reporting was locked back to the frozen `final_milestone_label_coarse` provider. Embedding Coherence was realigned to a stricter scTimeBench-style protocol: adapters now expose observed `embedding.npy` and projected `next_timepoint_embedding.npy`; old formal outputs can be backfilled without retraining; `eval_embedding_milestone.py` builds Leiden clusters from projected embeddings, transfers labels from observed embeddings by kNN, reports reference ARI only as diagnostics, and ranks the projected classifier entropy (`pred_tp_avg_normalized_entropy`) rather than hard-label cluster entropy. Shirokane launchers and the 2026-05-30 embedding bundle were prepared for backfill, official embedding reruns, and optional reference-ARI/sensitivity reruns.

The reporting layer was hardened so `summarize_official_silver.py` refuses to combine incompatible registry, embedding, lineage, and Leiden settings. Official reports now state the active contract explicitly (`state_key=final_milestone_label_coarse`, `n_neighbors=15`, `resolution=0.5`) and exclude expanded-sensitivity rows until the frozen provider and lineage benchmark are intentionally migrated. The regenerated official-silver ranking shows, among projection-capable methods on the primary chemical-reprogramming datasets, PRESCIENT first by combined rank (1.3333), MIOFlow second (1.4167), and scNODE third (1.7500); PRESCIENT leads Embedding Coherence, while graph-sim Lineage Fidelity is effectively tied under the current coarse graph. Forecast Accuracy was upgraded separately to exact scTimeBench OT-loss mode.

A new GSE175634 cardiac differentiation branch was added as an author-label silver benchmark. The canonical accession was fixed as `GSE175634` rather than `GSE174534`; preprocessing built a 224,202 x 2,000 HVG2000 h5ad with `abs_day` values `[0, 1, 3, 5, 7, 11, 15]`; and `gse175634_cardiac_silver_v1` was generated directly from the author `type` column. The provider has six official states and five reference edges (`IPSC -> MES -> CMES -> PROG -> CM` plus `PROG -> CF`), with 12,930 `UNK` cells excluded from official metrics. scNODE, MIOFlow, and PRESCIENT A/B/C runs completed, and WOT/CellRank2 A initially hit full-cell OT memory pressure but were repaired with per-timepoint stratified subsampling to 5,000 cells.

Lineage Fidelity was upgraded to scTimeBench graph-sim semantics across the active ground-truth outputs. `lineage_graphsim_sctimebench.py`, `eval_lineage.py`, and tests now cover single-step and all-path graph similarity, modified Floyd-Warshall reachability, diagonal/self-loop behavior, PRC/ROC thresholding, and legacy top-k Jaccard demotion to diagnostics. The rerun inventory records 40 completed `sctimebench_graph_sim` lineage results across GSE175634, GSE178325, GSE230659, and GSE242424; GSE175634 WOT and CellRank2 A reached perfect single-step graph recovery after the subsampling fix.

Forecast Accuracy was rerun in exact scTimeBench OT-loss mode from existing `projected_expression.npy` outputs. `eval_forecast.py` now records `metric_backend=scTimeBench_exact`, `metric_protocol=sctimebench_gex_prediction_otloss`, `exact_cell_usage=true`, gene-aligned prediction matrices, normalized Wasserstein/MMD metrics, and unnormalized Hausdorff loss, with tests for solver parity, exact/no-sampling metadata, sparse observed data, and gene reordering. The exact forecast inventory reports 36/36 completed configs across GSE175634, GSE178325, GSE230659, and GSE242424; the registered-provider report was regenerated after confirming 18 primary forecast rows, 20 lineage rows, and 18 embedding rows passed the active contract.

On the manuscript and design side, the silver-standard feasibility rationale was written to frame `official_silver` as a frozen benchmark reference rather than lineage-traced biological truth, with explicit limitations around time/library confounding, coarse milestone labels, ambiguous/OOD exclusions, and legacy scGPT circularity. A supplementary CLS-state D-F design plan was also drafted to replace inactive DPT pseudotime scenarios with frozen classifier-defined ordered states, keeping A-C as the primary observed-time benchmark and reporting CLS-state results separately if implemented later.

---

**2026.6.03–6.04 — Representation-dynamics records removed (v2.1, 2026-06-10).**
The scFM representation-dynamics entries (scGPT-CLS and Geneformer-CLS embeddings as
trajectory-model inputs) have been removed from the active benchmark per the v2.1
scope update (Section 1.1). The only model input is 2,000-HVG expression. The
`benchmark/results/representation_dynamics/` artifacts remain on disk but are
out-of-scope for the active design and are not reported.
