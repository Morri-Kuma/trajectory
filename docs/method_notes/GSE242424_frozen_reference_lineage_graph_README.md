...
Status: frozen ground truth provider provenance note

This note documents the frozen GSE242424 OSKM reprogramming reference graph and
its author-cluster-matched ground truth provider:

```text
benchmark/ground_truth/providers/gse242424_oskm_reprogramming_ground_truth_v1/
```
...
formal configs whose `result_class` is `gse242424_oskm_ground_truth_formal`.
...
Formal ground truth runs use the author-cluster-matched subset:
...
Formal GSE242424 OSKM ground truth runs use `label_mode=ground_truth`,
...
- The ground truth provider is based on author cluster transfer labels, not on an
